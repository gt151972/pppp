...
@implementation DKScrollTextLable
...
- (void)initUSerInterface {
    ...
    _distance = _rate>0 ? _rate * 2 : 1;
    _interval = _interval>0 ? _interval : 100;
    _style = DKTextCycleStyleDefault;
    [self creatLabel];
}
...
//        self.label1.text = self.label2.text = _text;
        [self.label1 setAttributedText:_attrStr];
        [self.label2 setAttributedText:_attrStr];
    }
...
//    if (_style == DKTextCycleStyleDefault && _textWidth < _width) {
//        [self stop];
//        return;
//    }
...
//            //当滑动界面时，系统为了更好地处理UI事件和滚动显示，主线程runloop会暂时停止处理一些其它事件
//            _timer = [NSTimer scheduledTimerWithTimeInterval:0.02 repeats:YES block:^(NSTimer * _Nonnull timer) {
//                [weakSelf labelChange];
//            }];
            _timer = [NSTimer timerWithTimeInterval:0.02 target:self selector:@selector(labelChange) userInfo:nil repeats:YES];
...
- (void)setStyle:(DKTextCycleStyle)style {
    _style = style;
    [self start];
}
