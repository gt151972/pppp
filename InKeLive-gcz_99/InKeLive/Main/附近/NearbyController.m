...
- (void)loadData{
    ...
    [[NetWorkTools shareInstance] getWithURLString:self.requestUrl parameters:nil success:^(NSDictionary *dictionary) {
        ...
        [self.nearCollectView.mj_header endRefreshing];  //停止刷新头
        [self.nearCollectView reloadData]; //重新载入数据
...
- (UICollectionView *)nearCollectView{
    if (!_nearCollectView) {
        ...
        layout.scrollDirection = UICollectionViewScrollDirectionVertical;  //方向?
