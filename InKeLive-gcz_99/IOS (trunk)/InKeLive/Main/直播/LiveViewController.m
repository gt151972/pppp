...
- (void)creatUI {
    ...
    [self.topSideView addSubview:self.membersHeadView];//观众头像
    [self.topSideView addSubview:self.onMicUsersHeadView];
    ...
    [self.giftView setGiftClick:^(NSInteger tag) {
        ...
        if(toId == 0) {
            ...
            DPK_NW_Application* dpk_app = [DPK_NW_Application sharedInstance];
            ClientGiftModel* model = [dpk_app.giftArray objectAtIndex:tag];
            if(model !=nil) {
...
-(RoomOnMicUsersView*)onMicUsersHeadView {
    if(!_onMicUsersHeadView) {
        CGRect frame = CGRectMake(SCREEN_WIDTH -64 -10, 100, 64, SCREEN_HEIGHT -100 -44 -44 - 64);
...
-(UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    else if(tableView == _onMicUsersHeadView)
    {
                        ...
                        GiftText:(NSString*)giftText
{
    ...
    ClientGiftModel* giftInfo = [[DPK_NW_Application sharedInstance] findGiftConfig:giftId];
