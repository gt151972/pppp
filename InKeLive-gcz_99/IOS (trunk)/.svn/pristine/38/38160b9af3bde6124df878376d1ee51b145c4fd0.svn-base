...
-(void) loadGiftConf
{
    ...
    NSString* strAPIUrl = [NSString stringWithFormat:@"%@%@",self.clientConfigParam.commonApiPrefix, URL_GetGiftConf];
    NSLog(@"url:%@", strAPIUrl);
    ...
    [manager POST:strAPIUrl parameters:parameters constructingBodyWithBlock:^(id<AFMultipartFormData> formData) {
        ...
        if([errorCode isEqualToString:@"0"])
        {
            ...
            NSArray* giftconflist = (NSArray*)appDic[@"data"];
            for(NSDictionary* giftItem in giftconflist) {

                ClientGiftModel* model = [[ClientGiftModel alloc] init];
                model.giftId = [giftItem[@"giftid"] intValue];
                model.giftName=giftItem[@"giftname"];
                model.effectType =[giftItem[@"effecttype"] intValue];
                model.coins = [giftItem[@"coins"] intValue];
                model.experience = [giftItem[@"experience"] intValue];
                model.fullscreen = [giftItem[@"fullscreen"] intValue];
                model.icon = [NSString stringWithFormat:@"%@%@", url_giftpic_prefix, giftItem[@"icon"]];
                model.isLan = [giftItem[@"islan"] intValue];
                model.isSelect = [giftItem[@"isselect"] intValue];
                model.isSupport = [giftItem[@"issupport"] intValue];
...
-(ClientGiftModel*) findGiftConfig:(int)giftId
{
    if(_giftArray != nil) {
        for(int i=0; i<_giftArray.count; i++) {
            if(((ClientGiftModel*)[_giftArray objectAtIndex:i]).giftId == giftId) {
                return [_giftArray objectAtIndex:i];
            }
        }
    }
    return nil;
